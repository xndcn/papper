...
- [x] **Step 7: 技能场景集成与赛间奖励**
  - [x] 更新 `src/scenes/BuildScene.ts`：新增技能装备功能
    - [x] 技能槽位面板（2-3 个槽位，根据飞机类型决定数量）
    - [x] 可用技能列表（显示已解锁的主动技能，含名称、图标占位、冷却时间、效果描述）
    - [x] 点击技能装入槽位 / 点击已装备技能卸下
    - [x] 实时属性预览中考虑技能的被动效果
  - [x] 更新 `src/scenes/RaceScene.ts`：集成主动技能激活与 Buff 显示
    - [x] 技能操作 UI：屏幕底部显示已装备的技能按钮（含冷却倒计时遮罩）
    - [x] 点击/触屏技能按钮触发技能激活，应用 Buff 到飞机物理参数
    - [x] 飞行 HUD 显示当前生效的 Buff 图标和剩余时间
    - [x] 被动技能自动触发检查（逆风触发、失速触发等），触发时显示提示
  - [x] 更新 `src/scenes/ResultScene.ts`：赛间奖励选择
    - [x] 比赛胜利后显示三选一奖励面板（随机生成：零件 / 金币 / 技能）
    - [x] 奖励数据来源于 TournamentSystem.completeRace 的返回值
    - [x] 选择奖励后更新 TournamentRun 状态（加入 collectedParts / runCoins / runSkills）
